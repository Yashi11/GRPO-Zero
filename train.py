...
from customdata_task import CustomDataset, reward_function
from grpo import rollout, update_policy
from optimizer import MemoryEfficientAdamW


def evaluate(model, tokenizer, vl_processor, device, dtype, config):
...
def main(config_path: str):
    ...
    # Initialize DeepSeek VL2 model and processor
    vl_chat_processor = DeepseekVLV2Processor.from_pretrained(pretrained_model_path)
    tokenizer = vl_chat_processor.tokenizer
    
    # Initialize datasets
    train_dataset = CustomDataset(
        vl_processor=vl_chat_processor,
        data_path=config["data"]["path"],
        split="train",
        test_size=config["data"]["test_size"],
    )
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        if step > config["training"].get("max_steps", float('inf')):
            print(f"\nReached max_steps ({config['training']['max_steps']}). Stopping training.")
            break
        ...
        if step % config["training"]["eval_interval"] == 0:
            eval_success_rate = evaluate(model, tokenizer, vl_chat_processor, device, dtype, config)
